...
public class ServerThread extends Thread{
    ...
    public void dealWithMsg(String msgReceived)
    {
        ...
        if (msgReceived.startsWith("/"))
        {
            ...
            { // 收到的信息为加入游戏时
                ...
                while (userTokens.hasMoreTokens())
                {
                    ...
                    {
                        playerNames[nameIndex - 1] = userToken; // 取游戏者命
                    }
                    nameIndex++;
                }
        ...
        else
        {
            ...
            serverMessagePanel.msgTextArea.append(msgReceived + "\n");
            sendPublicMsg(msgReceived);
            serverMessagePanel.msgTextArea.setCaretPosition(serverMessagePanel.msgTextArea.getText()
                    .length());
        }
    }
    ...
    {
        serverMessagePanel.msgTextArea.append("用户断开连接:" + clientSocket + "\n");
        synchronized (chessPeerHash)
        { //如果是游戏客户端主机
        ...
        sendPublicMsg(getUserList());
        serverMessagePanel.statusLabel.setText("当前连接数:" + clientDataHash.size());
        try
        {
            clientSocket.close();
        }
